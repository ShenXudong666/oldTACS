...
class TACS_model:
    def __init__(self, 
        ...
        if svm_path is not None and svm_acc is not None:
            ...
            self.layer_indices = self.sorted_indices
        self.window = 7
...
#本质上将该句子放进这个模型，但是不保存输出
            with torch.no_grad():
                output = self.model(**encodings)
            print(type(self.svm[0]))
            result = [torch.zeros(end_id[i]-start_id[i]) for i in range(len(start_id))] 

            for layer in self.layer_indices:
                #这里的model指的是LlamaForCausalLM，model.model指的是LlamaModel,layer指的是decoder层，
                #decoder层有分数
                attn_score = self.model.model.layers[layer].self_attn.attn_score
                for idx in range(len(start_id)):
                    ...
                    #最重要的事情是要清楚svm的内部结构
                    predict_result = self.svm[layer].predict(head_score)
                    result[idx] += predict_result
            ...
            for layer in self.sorted_indices:
                attn_score = self.model.model.layers[layer].self_attn.attn_score
